...
3. Create a new API client and copy the `client_id` and `client_secret` to the [APIConfiguration.h](firmware-original/config/APIConfiguration.h) file
...
8. Copy the generated key and add it to [APIConfiguration.h](firmware-original/config/APIConfiguration.h)
...
Enter your WiFi credentials into `WIFI_SSID` and `WIFI_PASSWORD` in [WiFiConfiguration.h](firmware-original/config/WiFiConfiguration.h)

### Set your location

Set your location to track flights by updating the following values in [UserConfiguration.h](firmware-original/config/UserConfiguration.h):
...
2. **Configure your settings**:
   - Add your API keys to [APIConfiguration.h](firmware-original/config/APIConfiguration.h)
   - Add your WiFi credentials to [WiFiConfiguration.h](firmware-original/config/WiFiConfiguration.h)
   - Set your location (and optional display preferences) in [UserConfiguration.h](firmware-original/config/UserConfiguration.h)
   - Adjust display hardware (pin, tile layout) in [HardwareConfiguration.h](firmware-original/config/HardwareConfiguration.h)
...
- **Brightness**: Controls overall display brightness (0–255)
  - Edit `DISPLAY_BRIGHTNESS` in [UserConfiguration.h](firmware-original/config/UserConfiguration.h)
- **Text color**: RGB values used for all text/borders
  - Edit `TEXT_COLOR_R`, `TEXT_COLOR_G`, `TEXT_COLOR_B` in [UserConfiguration.h](firmware-original/config/UserConfiguration.h)
