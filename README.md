...
Marine radar can draw a land/sea outline from a preprocessed OSMCoastline land-polygon extract. The Worker looks for `/marine/land-polygons.json` in Worker assets, or `MARINE_LANDMASK_URL` if configured. The source data must be trimmed to the product's operating region before deploy. At runtime the Worker converts the current rotated `126 x 46` radar rectangle to a compact `base64-land-bits-v1` mask and caches it in KV. Firmware and emulator draw set bits as black/off LEDs and leave unset bits as the configured radar background color.
