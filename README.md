...
- Text line 2 at `y=56`: `SOG` in rounded whole knots, `COG` with degree symbol and cardinal direction, then optional `Destination`.
- 1 px bottom margin.
...
Marine speed and direction are fixed to `SOG` and `COG`. If AIS does not provide a selected value, the display shows `--` for that field instead of substituting a different measurement.
